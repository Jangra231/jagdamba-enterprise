...
import { TIMELINE_DATA, CORE_VALUES, STATISTICS_DATA } from '../data';
...
function CountingNumber({ value, suffix }: { value: number; suffix: string }) {
  ...
  useEffect(() => {
    ...
    let start = 0;
    const end = value;
    const duration = 1500; // 1.5 seconds for snappier experience
  ...
  return (
    <span ref={elementRef} className="font-display font-black text-4xl sm:text-5xl text-[#0A0A0A]">
      {count}
      {suffix}
    </span>
  );
...
export default function About({ onNavigateMission, onNavigateVision }: AboutProps) {
  const [activeTimelineIdx, setActiveTimelineIdx] = useState(TIMELINE_DATA.length - 1);
  const containerRef = useRef(null);

  return (
    <section id="about" className="py-24 bg-[#FCFCFC]/50 relative overflow-hidden" ref={containerRef}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-left mb-16 max-w-3xl">
          <div className="flex items-center gap-2 mb-3">
            <span className="h-[2px] w-8 bg-[#0A0A0A]" />
          ...
          <p className="mt-4 text-gray-700 text-sm sm:text-base leading-relaxed">
            For over 25 years, Jagdamba Enterprises has been driven by a simple belief — that business can be a force for good. What began as a vision to deliver quality solutions has grown into a diversified enterprise creating sustainable value across industries and communities worldwide.
          </p>
        </div>

        {/* Brand Mission & Vision Grid Layout */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch mb-20">
          
          {/* Left building / graphic visual column */}
          <div className="lg:col-span-5 relative rounded-3xl overflow-hidden bg-white border border-[#0A0A0A]/10 p-8 flex flex-col justify-between group shadow-sm">
            <div className="absolute inset-0 bg-gradient-to-br from-[#056D34]/5 via-transparent to-[#0a0a0a]/5 pointer-events-none" />
            ...
            <div>
              ...
              <p className="text-gray-600 text-xs sm:text-sm mt-3 leading-relaxed">
                Jagdamba Enterprises is a trusted partner in procurement, partnerships, and performance. For over three decades, we've helped businesses across industries build strong connections, unlock growth, and achieve measurable results.
              </p>
            </div>

            {/* Simulated abstract geometric building visual */}
            <div className="my-8 relative flex items-center justify-center p-4">
              <svg className="w-full max-w-[280px] h-40 text-gray-200" viewBox="0 0 100 50" fill="none">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="bg-white border border-[#0A0A0A]/15 px-4 py-2 rounded-xl text-center shadow-sm">
                  <span className="font-mono text-[10px] tracking-widest text-gray-500 block uppercase">HEADQUARTERS</span>
                  <span className="text-xs font-bold text-[#0A0A0A] font-display">Mumbai & New Delhi</span>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-7 flex flex-col gap-6">
            ...
            >
              <div className="w-12 h-12 shrink-0 rounded-2xl bg-[#0A0A0A] flex items-center justify-center text-[#056D34] border border-[#0A0A0A]/10 group-hover:bg-[#056D34] group-hover:text-white transition-all">
                <Award size={24} />
              </div>
              <div className="text-left flex-grow">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-gray-500 text-xs font-bold tracking-widest font-mono">DRIVEN BY PURPOSE</span>
                  <span className="text-[10px] font-bold text-white bg-[#056D34] px-2 py-0.5 rounded font-mono">CLICK TO READ</span>
                </div>
                <h4 className="font-display font-extrabold text-xl text-[#0A0A0A] mb-2 group-hover:text-[#056D34] transition-colors">Our Mission</h4>
                <p className="text-gray-600 text-sm leading-relaxed">
                  To deliver integrated solutions that drive business growth through strategic partnerships, innovation, and unwavering commitment. We provide seamless, risk-free trade setups.
                </p>
              </div>
            ...
            >
              ...
              <div className="text-left flex-grow">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-gray-500 text-xs font-bold tracking-widest font-mono">GUIDED BY INTEGRITY</span>
                  <span className="text-[10px] font-bold text-white bg-[#056D34] px-2 py-0.5 rounded font-mono">CLICK TO READ</span>
                </div>
                <h4 className="font-display font-extrabold text-xl text-[#0A0A0A] mb-2 group-hover:text-[#056D34] transition-colors">Our Vision</h4>
                <p className="text-gray-600 text-sm leading-relaxed">
                  To be a globally respected enterprise known for creating value, building trust, and empowering businesses for the future. We aspire to make trading fully green.
                </p>
              </div>
              <div className="absolute right-6 top-6 sm:top-auto sm:bottom-6 w-8 h-8 rounded-full bg-[#FCFCFC] border border-[#0A0A0A]/10 flex items-center justify-center text-[#0A0A0A] group-hover:bg-[#056D34] group-hover:text-white transition-all">
                <ChevronRight size={16} />
                ...
                {CORE_VALUES.map((val, idx) => (
                  <div key={idx} className="flex gap-2.5 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-gray-500 leading-normal">{val.description}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* Counter Statistics Section Bar */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 bg-white border border-[#0A0A0A]/10 p-6 sm:p-8 rounded-3xl shadow-sm mb-24" id="stats-counter-bar">
          {STATISTICS_DATA.map((stat, index) => (
            <div key={index} className="flex flex-col items-center justify-center text-center p-4 border-r last:border-r-0 border-gray-200">
        ...
        {/* Growth Timeline: "Our Journey of Growth" */}
        <div className="bg-white border border-[#0A0A0A]/10 p-8 sm:p-12 rounded-3xl shadow-sm text-left">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              <span className="text-[10px] tracking-widest font-bold text-gray-500 block uppercase font-mono">HISTORICAL MILESTONES</span>
              <h3 className="font-display font-extrabold text-2xl text-[#0A0A0A]">Our Journey of Growth</h3>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-6 relative mb-8">
            {/* Timeline connector track bar */}
            <div className="hidden md:block absolute top-[28px] left-12 right-12 h-0.5 bg-gray-200 z-0" />
            ...
            {TIMELINE_DATA.map((evt, idx) => (
              ...
              >
                {/* Year dot */}
                <div className="flex items-center justify-center w-14 h-14 rounded-full border-2 transition-all duration-300 mb-3" style={{
                ...
                {/* Title */}
                <span className={`text-xs font-bold tracking-wider uppercase transition-colors duration-300 font-mono ${
                  activeTimelineIdx === idx ? 'text-[#056D34]' : 'text-gray-400 group-hover:text-[#056D34]'
                }`}>
                  {evt.title}
                </span>
              </button>
          ...
          >
            <div className="flex gap-4 items-center">
              <div className="w-12 h-12 rounded-xl bg-[#0A0A0A] text-[#056D34] flex items-center justify-center shrink-0 border border-[#0A0A0A]/10">
