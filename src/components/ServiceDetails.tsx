...
export default function ServiceDetails({ serviceId, onBack, onEnquire }: ServiceDetailsProps) {
  ...
  if (!service) {
    return (
      <div className="py-24 text-center max-w-xl mx-auto">
        ...
        <button onClick={onBack} className="mt-4 px-4 py-2 bg-[#0A0A0A] text-white rounded-xl">
          Back to Services
        </button>
      </div>
    );
  ...
  // Define some rich specifications / timeline based on service type
    ...
    switch (id) {
      case 'lead-generation':
        return {
          ...
          phases: [
            { title: 'Market Mapping & Ideal Buyer Profile', desc: 'Identify target volume buyers and procurement cycles.' },
            { title: 'Cold-to-Warm Outreach Funnel', desc: 'Active presentation of client trade capacities.' },
            { title: 'RFC (Request for Collaboration) Curation', desc: 'Compile precise raw material demand specifications.' },
            { title: 'Direct Handshake & Contract Facilitation', desc: 'Host secure initial calls and supply contract draftings.' }
          ],
          stats: [
      ...
      case 'trading':
        return {
          ...
          methodology: 'Validated sourcing hubs, maritime logistics, and letter of credit clearing',
          sla: 'Pricing contracts locked for 7 business days max',
          phases: [
            { title: 'Vendor Audit & Material QA', desc: 'Strict inspection of ore grades, mill reports, and certificates.' },
            { title: 'Incoterms Locking & Hedging', desc: 'Optimize CIF/FOB freight pricing against trade fluctuations.' },
            { title: 'Maritime Logistics & Shipping', desc: 'Vessel scheduling, container bookings, and port clearance.' },
            { title: 'Customs Clearance & Delivery', desc: 'Absolute document compliance for seamless border crossing.' }
          ],
          stats: [
      ...
      case 'consulting':
        return {
          ...
          methodology: 'Deep-dive feasibility studies, custom financial audits, and global ESG alignment maps',
          sla: 'Interim feasibility assessment drafted within 14 calendar days',
          phases: [
            { title: 'Scope Alignment & Briefing', desc: 'Establish commercial target bounds, budgets, and compliance goals.' },
            { title: 'Macro Commodity Research', desc: 'Analyze historical price actions, supply gaps, and policy risks.' },
            { title: 'Financial Risk Feasibility Analysis', desc: 'Formulate precise IRR, NPV, and capital amortization curves.' },
            { title: 'Deliverable Submission & Board Review', desc: 'Present actionable strategic roadmap with complete audits.' }
          ],
          stats: [
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10 text-left">
      ...
      {/* Main Grid: Header Card and Specs Card */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mb-12">
        
        {/* Left Column: Brief and Overview */}
        <div className="lg:col-span-7 space-y-8">
          <div className="bg-white border border-[#0A0A0A]/10 p-8 sm:p-10 rounded-3xl shadow-sm">
            ...
            <div className="grid grid-cols-3 gap-4 border-t border-[#0a0a0a]/10 pt-6">
              {ext.stats.map((st, idx) => (
                <div key={idx} className="bg-gray-50 p-4 rounded-xl border border-[#0a0a0a]/5 text-center">
                  <p className="font-display font-black text-2xl text-[#0A0A0A]">{st.value}</p>
                  <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wider font-mono mt-1 leading-tight">{st.label}</p>
                </div>
              ))}
            </div>
          ...
          {/* Sourcing Timeline Process / Phase */}
          <div className="bg-white border border-[#0A0A0A]/10 p-8 sm:p-10 rounded-3xl shadow-sm text-left">
            ...
            <p className="text-xs text-gray-500 mb-8 font-normal">
              Our systematic approach guarantees zero delays, compliance locking, and complete quality checks.
            </p>
            ...
            <div className="relative border-l border-[#0A0A0A]/10 ml-4 space-y-8">
              {ext.phases.map((phase, idx) => (
                <div key={idx} className="relative pl-8">
                  ...
                  <div>
                    <h4 className="font-display font-extrabold text-sm sm:text-base text-[#0A0A0A]">{phase.title}</h4>
                    <p className="text-xs sm:text-sm text-gray-600 mt-1 font-normal leading-relaxed">{phase.desc}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Right Column: Parameters, Auditing & Immediate CTA */}
        <div className="lg:col-span-5 space-y-8">
          ...
          <div className="bg-white border border-[#0A0A0A]/10 p-8 rounded-3xl shadow-sm text-left">
            ...
            <div className="space-y-4">
              <div>
                <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider font-mono">Target Audience</span>
          ...
          <div className="bg-white border border-[#0A0A0A]/10 p-8 rounded-3xl shadow-sm text-left">
            ...
            <div className="flex flex-col gap-3">
              {service.details.map((detail, idx) => (
                <div key={idx} className="flex gap-2 items-center bg-gray-50 border border-[#0A0A0A]/5 px-4 py-3 rounded-xl">
          ...
          <div className="bg-[#0A0A0A] text-white p-8 rounded-3xl shadow-lg text-center relative overflow-hidden">
            ...
            <p className="text-xs text-gray-300 mb-6 leading-relaxed">
              Enquire today and connect with our specialized global sourcing panel for a custom RFC.
            </p>
            <button
